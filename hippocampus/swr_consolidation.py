...
class SWRConsolidation:
    ...
    def _run_consolidation(self):
        ...
        # ========== 2.5 记忆分层固化（新增：模拟人脑睡眠巩固）==========
        # 空闲时执行记忆的层级转换和衰减
        if hasattr(self.hippocampus, 'consolidate_memories'):
            try:
                consolidation_stats = self.hippocampus.consolidate_memories()
                logger.debug(f"[SWR] 记忆固化完成: {consolidation_stats}")
            except Exception as e:
                logger.debug(f"[SWR] 记忆固化失败: {e}")
