...
class ReplayBuffer:
    ...
    def __init__(self, capacity: int = 100):
        ...
        self.buffer: List[ReplaySequence] = []
    
    def push(self, sequence: ReplaySequence):
        """推入新序列"""
    ...
    def sample(self, batch_size: int, priority: bool = True) -> List[ReplaySequence]:
        """
        采样回放序列
        
        Args:
            batch_size: 采样数量
            priority: 是否优先采样高奖励序列
        ...
        if priority:
            # 按奖励信号加权采样
            weights = [s.reward_signal + 0.01 for s in self.buffer]  # 加小值避免除零
