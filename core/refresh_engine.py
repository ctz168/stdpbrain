    ...
    def _generate_memory_gate_signal(
        ...
        if memory_anchors:
            ...
            if anchor_features:
                ...
                gate_signal = torch.zeros(1024, device=self.device)
                for feat, weight in zip(anchor_features, weights):
                    gate_signal += feat * weight / total_weight
                return gate_signal
    ...
    def _extract_features(self, token_id: int) -> torch.Tensor:
        ...
        if features is None:
            ...
            if hidden_size and hasattr(hidden_size, 'hidden_size'):
                ...
                hidden_size = 1024  # 默认 Qwen hidden size
    ...
    def _build_attention_gate(self, memory_anchors: List[dict]) -> Optional[torch.Tensor]:
        ...
        if hasattr(self.hippocampus, 'generate_attention_gate'):
            try:
                ...
                hidden_size = 1024  # Qwen hidden size
