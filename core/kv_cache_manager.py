...
class KVCacheManager:
    ...
    def trim_kv_cache(
        ...
        trim_threshold = int(self.window_size * 2.0)  # 从1.5放宽到2.0，减少不必要的KV修剪
    ...
    def _extract_evicted_kv(
        ...
        优化：使用 narrow 代替 slice+clone，减少内存分配
        """
        evicted = []
        
        for k, v in past_key_values:
            evict_len = k.shape[2] - window_size
            if evict_len <= 0:
                continue
            # 使用 narrow 避免创建完整副本
            evicted_k = k[:, :, :evict_len, :].contiguous()
            evicted_v = v[:, :, :evict_len, :].contiguous()
    ...
    def _keep_window_kv(
        ...
        for k, v in past_key_values:
            ...
            # 使用 contiguous() 而非 clone()，避免完整内存拷贝
            trimmed_k = k[:, :, start:, :].contiguous()
            trimmed_v = v[:, :, start:, :].contiguous()
