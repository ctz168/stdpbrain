...
class CompetitionMechanism(nn.Module):
    ...
    def forward(
        ...
        for output in outputs:
            inp = output.output.to(target_dtype) if output.output.dtype != target_dtype else output.output
            score = self.importance_net(inp)
        ...
        if context is not None and context.numel() > 0:
            ctx = context.to(target_dtype) if context.dtype != target_dtype else context
            relevance_scores = []
            for output in outputs:
                out = output.output.to(target_dtype) if output.output.dtype != target_dtype else output.output
...
class BroadcastMechanism(nn.Module):
    ...
    def forward(
        ...
        # 编码
        target_dtype = next(self.parameters()).dtype
        inp = winner_output.to(target_dtype) if winner_output.dtype != target_dtype else winner_output
        encoded = self.broadcast_encoder(inp)
...
class GlobalWorkspace:
    ...
    def __init__(
        self,
        hidden_size: int = 1024,  # 应由调用方从模型配置动态传入
        device: str = "cpu",
        max_history: int = 100
    ):
    ...
    def to(self, dtype=None, device=None):
        ...
        因为 GlobalWorkspace 本身不是 nn.Module，需要手动转发。
        """
        if dtype is not None:
            self.competition = self.competition.to(dtype=dtype)
            self.broadcast = self.broadcast.to(dtype=dtype)
            self._dimension_adapters = self._dimension_adapters.to(dtype=dtype)
        if device is not None:
            self.competition = self.competition.to(device=device)
            self.broadcast = self.broadcast.to(device=device)
            self._dimension_adapters = self._dimension_adapters.to(device=device)
        return self
