...
class SelfStateEncoder(nn.Module):
    ...
    将底层隐藏状态 (hidden_size-dim) 投影为"自我感知"向量 (256-dim)，
    然后再投影回 hidden_size-dim 以便注入生成上下文。
    ...
    def __init__(self, hidden_size: int = 2048, device: str = "cpu"):
        """
        Args:
            hidden_size: 模型隐藏层维度（应由调用方从模型配置动态传入）
            device: 设备
        """
        super().__init__()
    ...
    def encode(self, hidden_state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 确保在正确设备上，dtype 与 encoder 权重一致
        target_dtype = next(self.encoder.parameters()).dtype
        hidden_state = hidden_state.to(self.device, dtype=target_dtype)
